...
const AIRLINES: Record<string, AirlineCompany> = {
  ...
  U6: {
    ...
    logo: "https://logo.clearbit.com/uralairlines.ru",
  },
...
const randomDeparture = (date: string): string => {
...
export const generateOffers = (options: GenerateOptions = {}): Offer[] => {
  const { count = 50, date = "2025-08-15", from = "ALA", to = "SVO", maxStops = 2 } = options;

  const offers: Offer[] = [];
  const airlineCodes = Object.keys(AIRLINES) as (keyof typeof AIRLINES)[];

  // Хранилище для маршрутов, чтобы клонировать их (создавать дубли)
  const existingRoutes: Segment[][] = [];
  const duplicateChance = 0.2; // 20% шанс создания дубликата
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Решаем: создать новый уникальный маршрут или дубликат существующего
    const shouldCreateDuplicate = existingRoutes.length > 0 && Math.random() < duplicateChance;

    if (shouldCreateDuplicate) {
      // СОЗДАЕМ ДУБЛИКАТ: Берем случайный уже созданный набор сегментов
      segments = pick(existingRoutes);
      totalStops = segments.length - 1;
    } else {
      // СОЗДАЕМ УНИКАЛЬНЫЙ МАРШРУТ (старая логика)
      ...
      // Сохраняем новый уникальный маршрут в базу для будущих дублей
      existingRoutes.push(segments);
    }

    // ── Цена и условия (всегда генерируются заново, даже для дублей) ──────────

    // Для дублей имитируем "другую цену" от другого агентства
    const basePrice =
      totalStops === 0
